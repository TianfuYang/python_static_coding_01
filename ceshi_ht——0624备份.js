...
function bigImg_out2(x) {
	...
	//console.log('显示小程序码提示文字')
	// alert("长按识别下方小程序码即可前往练习哦")
	var say_hi=["~很高兴见到你哦~","~要不点击代码框前的文字看一下？~","点击右上角的导航键可以私信哦","--欢迎大家提出宝贵建议--","~每天的夕阳也会有变化呢~","~遇到问题不要慌~","~优雅、简洁、明确~","~踩着bug的肩膀成长~","~你好棒呀~","~一定要坚持哦~","~你想知道我有多少变化吗~","^_^"]
...
function check_libs() {
	...
	document.getElementById(3).getElementsByClassName("cm-content")[0].innerText=decodeURI(window.location.href.slice(cur_url.indexOf("&")+1,cur_url.lastIndexOf("&")))
	}
	// 如果url里没有参数 那什么也不做
	if (cur_url.indexOf("?") != -1) {
...
function load_new_lib(x) {
	...
	// var txt;
	var cur_url = window.location.hostname.length>1?window.location.hostname + window.location.pathname:window.location.href
	// var lib_names = prompt("请输入您需要的第三方python库的名字,不同的库用+号连接 例如：", "numpy+matplotlib");
	if (lib_names == null || lib_names == "") {
		console.log("用户取消输入")
		alert("您没有输入哦")
...
function copy_code(id) {
	...
	sum_show_num= sum_show_num-1
	console.log('跳转链接',cur_url+ '?' + code_txt)
	window.open(cur_url + '?&' + code_txt+'&', '_blank')
